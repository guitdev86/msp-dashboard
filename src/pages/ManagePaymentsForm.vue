  ...
  <div class="row">
    ...
    <div class="col-md-8">
      <table>
        ...
        <tbody>
          <table-line-container v-for="payment in this.paymentsList" 
                                :key="payment._id"
                                :payment="payment"
    ...
    <div class="col-md-4">
      <student-list v-on:studentSelected="filterStudentData" 
  ...
  export default {
    ...
    methods: {
      ...
      filterStudentData(name) {
        ...
        this.model.studentId = this.studentList.filter(student => student.name === name)[0].id;
        this.paymentsList = this.paymentsList.filter(student => student.studentId === this.model.studentId);
      },

      deletePaymentById(id) {
        //this.deletePayment(id);
        this.fetchPayments();
        this.paymentsList = this.paymentsList.filter(payment => payment._id !== id);
      ...
      editPaymentById(data) {
        ...
        this.paymentsList.forEach(payment => {
         if(payment._id === data._id) {
           ...
           payment.paymentAmount = parseInt(data.paymentAmount);
         };
        })
        // TODO HERE PLEASE!!!
        //if date is updated
        // if(String(data[1]).split('-').length < 3) {
