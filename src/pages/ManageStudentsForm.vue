<template>
  <div class="row">
    <div class="col-md-8">
      ...
      <div class="">
        <base-button slot="footer" type="primary" fill @click="saveChangesToDb">Save</base-button>
        <base-button class="ml-2" slot="footer" type="primary" fill>Delete</base-button>
      </div>
    </div>
    <div class="col-md-4">
      <student-list v-on:studentSelected="readData" 
  ...
  export default {
    ...
    methods: {
      ...
      editIsFinished() {
        ...
        paragraphContainers.forEach(paragraphContainer => paragraphContainer.$children.forEach(container => componentsNames.push(container.$vnode.tag)));
      ...
      async saveChangesToDb() {
        ...
        if(!this.isFilled(firstName) || !this.isFilled(lastName) || !this.isFilled(hourlyRate)) {
          this.notifyVue(EmptyFieldNotification, 4);
        } else if(!this.isLetter(firstName) || !this.isLetter(lastName) || !this.isNumber(hourlyRate)) {
      ...
      async readData(data) {
        ...
        let studentObject = await this.getStudentObject(this.model.firstName, this.model.lastName);
        ...
        tableData.forEach(element => this.$t('dashboard.manageInfo').push(element));
